...
if (strlen($rawInput) > MAX_BYTES) {
    echo json_encode(['error' => 'Audio too large']);
    exit;
}

$tmpWav = TMP_DIR . '/rec_' . uniqid('', true) . '.wav';
...
// ── Extract JSON from output (ignore any TF/librosa noise) ───────────────────
$json = null;
foreach (explode("\n", trim($output)) as $line) {
    $line = trim($line);
...
if (!$json) {
    echo json_encode(['error' => 'No output from Python', 'detail' => trim($output)]);
    exit;
}
...
if (json_last_error() !== JSON_ERROR_NONE) {
    echo json_encode(['error' => 'Invalid JSON from Python', 'detail' => $json]);
    exit;
}
